...
void main() {
  ...
  test('a whole capture refuses on the same terms as one page', () {
    expect(
      () => shapeScannedPages(
        pages: const [(lines: [line], width: 1000.0, height: 1400.0)],
      ),
  ...
  test('the rest of the scanning surface refuses too, rather than guessing', () {
    const layer = (
      source: 'camera',
    ...
    for (final call in <void Function()>[
      () => buildTextLayer(
        pages: const [(lines: [line], width: 0.0, height: 0.0)],
        source: 'camera',
      ),
      ...
      () => planPagePrepare(
        ...
        sample: (histogram: Uint32List(256)),
        width: 1000.0,
        height: 1400.0,
      ),
      ...
      () => choosePageReading(
        readings: const [(lines: [line], width: 1000.0, height: 1400.0)],
      ),
    ]) {
